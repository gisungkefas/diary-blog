...
public class CategoryServiceImpl implements CategoryService {
    ...
    public ResponseEntity<Category> getCategory(Long id) {
        ...
        Category category = categoryRepository.findById(id).orElseThrow(
                ()-> new ResourceNotFoundException("Category Id was not found")
        );
    ...
    public ResponseEntity<Category> updateCategory(Long id, Category newCategory, UserPrincipal currentUser) throws UnauthorizedException {
        ...
        if (category.getCreateDate().equals(currentUser.getId()) || currentUser.getAuthorities().contains(new SimpleGrantedAuthority(Role.ADMIN.toString()))) {
    ...
    public ResponseEntity<ApiResponse> deleteCategory(Long id, UserPrincipal currentUser) throws UnauthorizedException {
        ...
        if (category.getCreateDate().equals(currentUser.getId()) || currentUser.getAuthorities()
            ...
            return new ResponseEntity<>(new ApiResponse("You have successfully deleted a category", LocalDateTime.now()), HttpStatus.OK);
        }
        throw new UnauthorizedException("You don't have permission to perform this operation");
    }
}
